...
local function checkForUpdate()
    ...
    -- Через прямую запись с указанием кодировки
    local newFile = io.open(scriptPath, "w")
    if newFile then
        -- Конвертируем содержимое из UTF-8 в ANSI
        local ansiContent = newContent  -- Пока просто копируем
        -- Пробуем через encoding (если есть)
        local encoding = require("encoding")
        if encoding then
            encoding.default = "CP1251"
        ...
        newFile:close()
        os.remove(tempPath)
        print("[FastHelperAdm] Файл успешно заменён на 2.2 в ANSI!")
    else
