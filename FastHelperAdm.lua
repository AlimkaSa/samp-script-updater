...
local function checkForUpdate()
    ...
    -- Åñòü îáíîâëåíèå!
    print(string.format("[FastHelperAdm] Íàéäåíî îáíîâëåíèå! %s -> %s", CURRENT_VERSION, remoteNum))
    
    -- Ñêà÷èâàåì â temp ôàéë
    if not downloadFile(repoURL .. "/" .. scriptName, tempPath) then
        print("[FastHelperAdm] Îøèáêà ñêà÷èâàíèÿ!")
        return
    end
    
    -- Ïðîâåðÿåì, ÷òî ñêà÷àëîñü
    local testFile = io.open(tempPath, "r")
    if not testFile then
        print("[FastHelperAdm] Ôàéë íå ñêà÷àí!")
        return
    end
    testFile:close()
    
    -- Ïðîâåðÿåì, ÷òî â ñêà÷àííîì ôàéëå ïðàâèëüíàÿ âåðñèÿ
    local newContent = io.open(tempPath, "r"):read("*a")
    if not newContent or not newContent:find("CURRENT_VERSION = 2.2") then
        print("[FastHelperAdm] Îøèáêà: ñêà÷àííûé ôàéë ñîäåðæèò ñòàðóþ âåðñèþ!")
        os.remove(tempPath)
        return
    end
    ...
    -- ===== ÊÎÍÂÅÐÒÀÖÈß Â ANSI (Windows-1251) =====
    -- ×åðåç ïðÿìóþ çàïèñü ñ óêàçàíèåì êîäèðîâêè
    local newFile = io.open(scriptPath, "w")
    if newFile then
        -- Êîíâåðòèðóåì ñîäåðæèìîå èç UTF-8 â ANSI
        local ansiContent = newContent  -- Ïîêà ïðîñòî êîïèðóåì
        -- Ïðîáóåì ÷åðåç encoding (åñëè åñòü)
        local encoding = require("encoding")
        if encoding then
            encoding.default = "CP1251"
            local utf8 = encoding.UTF8
            local ansi = encoding.ANSI
            ansiContent = ansi:decode(utf8:encode(newContent))
        end
        newFile:write(ansiContent)
        newFile:close()
        os.remove(tempPath)
        print("[FastHelperAdm] Ôàéë óñïåøíî çàìåí¸í íà 2.2 â ANSI!")
    else
        print("[FastHelperAdm] Îøèáêà çàìåíû ôàéëà!")
        os.rename(backupPath, scriptPath)
        return
    end
    
    print("[FastHelperAdm] Îáíîâëåíèå óñòàíîâëåíî íà âåðñèþ " .. remoteNum .. "!")
    printStringNow("~g~FastHelperAdm~w~: ~y~Îáíîâëåíèå óñòàíîâëåíî!~n~~w~Âåðñèÿ " .. remoteNum, 3000)
    ...
    lua_thread.create(function()
        ...
        for _, scr in ipairs(script.list()) do
            if scr.filename == scriptPath or scr.filename:match("FastHelperAdm%.lua$") then
                print("[FastHelperAdm] Âûãðóæàþ ñêðèïò: " .. scr.filename)
                scr:unload()
                break
            end
        end
...
function adminRender.onServerMessage(color, text)
    ...
    -- Ïðîïóñêàåì çàãîëîâîê
    if cleanText:find("Àäìèíèñòðàöèÿ â ñåòè:") or cleanText:find("Àäìèíèñòðàòîðû îíëàéí") then
        return true
    end
    
    -- Ïàðñèì: "Nickname[ID] (LVL lvl) [AFK]"
    local nick, id, lvl, action = 
        cleanText:match("([^%[]+)%[(%d+)%] %((%d+) lvl%)(.*)")
    
    -- Àëüòåðíàòèâíûé ôîðìàò: "Nickname [ID] - LVL lvl"
    if not nick then
        nick, id, lvl, action = 
            cleanText:match("([^%[]+)%[(%d+)%] %- (%d+) lvl(.*)")
    end
...
function adminRender.draw()
    ...
    -- Åñëè àäìèíîâ íåò, ïîêàçûâàåì ñòàòóñ
    if #adminRender.admins == 0 then
        local posX = adminRender.posX
        local posY = adminRender.posY
        renderFontDrawText(adminRender.font, "Àäìèíèñòðàòîðû: çàãðóçêà...", posX, posY, -1)
        return
    end
    ...
    local header = string.format("Àäìèíèñòðàòîðû {00ff00}online{ffffff} [ %s | {ff0000}AFK: %s{ffffff} | {32CD32}/re: %s{ffffff} ]:", 
...
-- ===== ÔÓÍÊÖÈß ÇÀÃÐÓÇÊÈ ÒÅÊÑÒÎÂÛÕ ÔÀÉËÎÂ =====
function FHA_loadTextFile(path)
    local t = {}
    local f = io.open(path, "r")
    if not f then 
        table.insert(t, "Ôàéë íå íàéäåí: " .. path)
        return t
    end
...
FHA.rulesEnvyNames = {
    "ENVY | Ïðàâèëà Ñåðâåðà",
    "ENVY | Ïðàâèëà Àäìèíèñòðàöèè",
    "ENVY | Ïðàâèëà èñïîëüçîâàíèÿ /aad è /o",
    "ENVY | Îáùèå ïðàâèëà Goss",
    "ENVY | Ïðàâèëà êàïòîâ",
    "ENVY | Ïðàâèëà ñòðåë bizwar"
}

FHA.rulesPrideNames = {
    "PRIDE | Ïðàâèëà Ñåðâåðà",
    "PRIDE | Ïðàâèëà Àäìèíèñòðàöèé",
    "PRIDE | Ïðàâèëà Èñïîëüçîâàíèÿ /aad è /o",
    "PRIDE | Îáùèå ïðàâèëà Goss",
    "PRIDE | Ïðàâèëà Êàïòîâ",
    "PRIDE | Ïðàâèëà ñòðåë bizwar"
}

FHA.rulesAngerNames = {
    "ANGER | Ïðàâèëà Ñåðâåðà",
    "ANGER | Ïðàâèëà Àäìèíèñòðàöèè",
    "ANGER | Ïðàâèëà èñïîëüçîâàíèÿ /aad è /o",
    "ANGER | Îáùèå ïðàâèëà Goss",
    "ANGER | Ïðàâèëà êàïòîâ",
    "ANGER | Ïðàâèëà ñòðåë bizwar"
}
...
-- ===== ZZVEH ÔÓÍÊÖÈß (îáõîä çåë¸íîé çîíû) =====
function FHA_createCarInZZ()
    local st = FHA.state
    lua_thread.create(function()
        if FHA.isUnloading then return end
        
        -- Çàïîìèíàåì ïîçèöèþ èãðîêà
        ...
        -- Îòïðàâëÿåì /veh ñ ñîõðàí¸ííûìè ïàðàìåòðàìè
        sampSendChat("/veh " .. st.zzvehId .. " " .. st.zzvehC1 .. " " .. st.zzvehC2)
        wait(500)
        
        st.zzvehAct = false
        
        -- Âîçâðàùàåì èãðîêà íà ìåñòî
...
-- ===== ÔÓÍÊÖÈÈ ÂÈÇÓÀËÜÍÎÉ ÏÎ×ÈÍÊÈ ÌÀØÈÍÛ =====
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    ...
    if st.mpAutoStep == 1 and title:find(u8:decode("Ìåíþ ìåðîïðèÿòèé")) then
        lua_thread.create(function()
            if FHA.isUnloading then return end
            wait(200)
            if FHA.isUnloading then return end
    ...
    if st.otborRunning and title:find(u8:decode("Ìåíþ ìåðîïðèÿòèé")) then
        lua_thread.create(function()
            if FHA.isUnloading then return end
            wait(200)
            if FHA.isUnloading then return end
...
-- ===== ÎÑÍÎÂÍÎÉ ÎÁÐÀÁÎÒ×ÈÊ ÑÎÎÁÙÅÍÈÉ =====
function sampev.onServerMessage(color, text)
    -- === ÏÅÐÅÕÂÀÒ /VEH ÄËß ZZVEH ===
    local st = FHA.state
    
    -- Óáèðàåì öâåòîâûå êîäû
    local rawText = text:gsub("{%x%x%x%x%x%x}", "")
    
    -- Ïðîâåðÿåì êîìàíäó /veh (3 ïàðàìåòðà: id, c1, c2)
    if rawText:find("^/veh%s(%d+)%s(%d+)%s(%d+)") and not st.zzvehAct and st.zzvehEnabled.v and st.adminLevel.v >= 6 then
        -- Ñîõðàíÿåì ïàðàìåòðû êàê â GrandTools
        ...
        -- Íå ïîêàçûâàåì êîìàíäó â ÷àòå
        return false
    end
    
    -- Ïðîâåðÿåì âûõîä èç çîíû (êàê â GrandTools)
    if rawText:find("Âû ïîêèíóëè çåë¸íóþ çîíó") or rawText:find("Âû ïîêèíóëè çîíó") then
        if st.zzvehActive and st.zzvehEnabled.v and st.adminLevel.v >= 6 then
            -- Åñëè ìàøèíà åù¸ íå ñîçäàëàñü, ñîçäà¸ì ïðè âûõîäå
    ...
    if state.autoWish.v then
        if cleanText:find("ÁÀÍÊÎÂÑÊÈÉ ×ÅÊ") then
        ...
        elseif not cleanText:find("ÁÀÍÊÎÂÑÊÈÉ ×ÅÊ") then
    ...
    nick, id, msg = cleanText:match("Ðåïîðò îò (.+)%[(%d+)%]: (.+)")
        ...
        if msg2 then
            ...
            if repWord == u8:decode(state.text_word.v) then
                if sampIsPlayerConnected(tonumber(pid2)) then
                    ...
                    sampAddChatMessage("{33FF33}[FastHelperAdm] Ïîáåäèòåëü íàéäåí! Âûäà÷à ïðèçà...", -1)
                else
                    sampAddChatMessage("{FF5555}[FastHelperAdm] Ïîáåäèòåëü âûøåë èç èãðû, ðàçäà÷à îòìåíåíà", -1)
                    FHA_resetRazdacha()
                end
            end
        end
...
function FHA_adminWord()
    if FHA.state.gender.v == 1 then
        return "Àäìèíèñòðàòîðøè"
    else
        return "Àäìèíèñòðàòîðà"
    end
end
...
FHA.templates = {
    pleasant_game = function()
        return FHA_genderText(
            "Ïðèÿòíîé èãðû îò Àäìèíèñòðàòîðà <3",
            "Ïðèÿòíîé èãðû îò Àäìèíèñòðàòîðøè <3"
        )
    end,

    pleasant_game_waiting = function()
        return FHA_genderText(
            "Îæèäàéòå | Ïðèÿòíîé èãðû îò Àäìèíèñòðàòîðà <3",
            "Îæèäàéòå | Ïðèÿòíîé èãðû îò Àäìèíèñòðàòîðøè <3"
        )
    end,
    ...
    helped = function()
        return FHA_genderEnding(
            "ß ïîìîã(ëà) âàì. " .. FHA.templates.pleasant_game()
        )
    end,

    waiting = function()
        return FHA_genderEnding(
            "Îæèäàéòå, ÿ óæå ïðîâåðÿþ âàøó ñèòóàöèþ."
        )
    end,

    watching = function()
        return FHA_genderEnding(
            "ß ñëåæó çà âàøåé ñèòóàöèåé."
        )
    end,

    transferred = function()
        return FHA_genderEnding(
            "Ïåðåäàþ âàø ðåïîðò ñòàðøåé àäìèíèñòðàöèè."
        )
    end,

    spawn = function()
        return "Èñïîëüçóéòå /spawn äëÿ ðåøåíèÿ ïðîáëåìû."
    end
}
...
-- ===== ÂÑÏÎÌÎÃÀÒÅËÜÍÛÅ ÔÓÍÊÖÈÈ ÄËß ÌÅÍÞ =====
...
local function DrawTracerSettings()
    ...
    imgui.TextColored(cs.accent, u8"=== ÍÀÑÒÐÎÉÊÈ ÒÐÅÉÑÅÐÀ ===")
    imgui.Spacing()
    
    imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"Ìîè ïóëè:")
    if imgui.Checkbox(u8"Îòîáðàæàòü ñâîè ïóëè", st.tracerDrawMyBullets) then st.saveSettingsFlag = true end
    if imgui.Checkbox(u8"Ïîëèãîí â êîíöå (ñâîè)", st.tracerCbEndMy) then st.saveSettingsFlag = true end
    
    imgui.SliderInt(u8"Âðåìÿ îòîáðàæåíèÿ (ñâîè)", st.tracerTimeRenderMyBullets, 1, 60)
    imgui.SliderInt(u8"Òîëùèíà ëèíèè (ñâîè)", st.tracerSizeOffMyLine, 1, 10)
    imgui.SliderInt(u8"Ðàçìåð ïîëèãîíà (ñâîè)", st.tracerSizeOffMyPolygonEnd, 1, 20)
    imgui.SliderInt(u8"Âðàùåíèå ïîëèãîíà (ñâîè)", st.tracerRotationMyPolygonEnd, 1, 360)
    imgui.SliderInt(u8"Óãîë ïîëèãîíà (ñâîè)", st.tracerDegreeMyPolygonEnd, 3, 360)
    imgui.SliderInt(u8"Ëèìèò ëèíèé (ñâîè)", st.tracerMaxLineMyLimit, 1, 100)
    
    imgui.ColorEdit4(u8"Ñòàòèê. îáúåêò (ñâîè)", st.tracerStaticObjectMy)
    imgui.ColorEdit4(u8"Äèíàìèê. îáúåêò (ñâîè)", st.tracerDinamicObjectMy)
    imgui.ColorEdit4(u8"Èãðîê (ñâîè)", st.tracerPedPMy)
    imgui.ColorEdit4(u8"Ìàøèíà (ñâîè)", st.tracerCarPMy)
    
    imgui.Separator()
    imgui.Spacing()
    
    imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"×óæèå ïóëè:")
    if imgui.Checkbox(u8"Îòîáðàæàòü ÷óæèå ïóëè", st.tracerDrawBullets) then st.saveSettingsFlag = true end
    if imgui.Checkbox(u8"Ïîëèãîí â êîíöå (÷óæèå)", st.tracerCbEnd) then st.saveSettingsFlag = true end
    if imgui.Checkbox(u8"Èíôîðìàöèÿ îá èãðîêå", st.tracerShowPlayerInfo) then st.saveSettingsFlag = true end
    
    if st.tracerShowPlayerInfo.v then
        if imgui.Checkbox(u8"Òîëüêî ID", st.tracerOnlyId) then st.saveSettingsFlag = true end
        imgui.SameLine()
        if imgui.Checkbox(u8"Òîëüêî Íèê", st.tracerOnlyNick) then st.saveSettingsFlag = true end
    end
    
    imgui.SliderInt(u8"Âðåìÿ îòîáðàæåíèÿ (÷óæèå)", st.tracerTimeRenderBullets, 1, 60)
    imgui.SliderInt(u8"Òîëùèíà ëèíèè (÷óæèå)", st.tracerSizeOffLine, 1, 10)
    imgui.SliderInt(u8"Ðàçìåð ïîëèãîíà (÷óæèå)", st.tracerSizeOffPolygonEnd, 1, 20)
    imgui.SliderInt(u8"Âðàùåíèå ïîëèãîíà (÷óæèå)", st.tracerRotationPolygonEnd, 1, 360)
    imgui.SliderInt(u8"Óãîë ïîëèãîíà (÷óæèå)", st.tracerDegreePolygonEnd, 3, 360)
    imgui.SliderInt(u8"Ëèìèò ëèíèé (÷óæèå)", st.tracerMaxLineLimit, 1, 100)
    
    imgui.ColorEdit4(u8"Ñòàòèê. îáúåêò (÷óæèå)", st.tracerStaticObject)
    imgui.ColorEdit4(u8"Äèíàìèê. îáúåêò (÷óæèå)", st.tracerDinamicObject)
    imgui.ColorEdit4(u8"Èãðîê (÷óæèå)", st.tracerPedP)
    imgui.ColorEdit4(u8"Ìàøèíà (÷óæèå)", st.tracerCarP)
    imgui.ColorEdit4(u8"Öâåò èíôî èãðîêà", st.tracerColorPlayerI)
    
    imgui.Separator()
    imgui.Spacing()
    
    if imgui.Button(u8"Ñîõðàíèòü íàñòðîéêè", imgui.ImVec2(-1, 30)) then
        FHA_saveCfg()
        sampAddChatMessage("{33FF33}[FastHelperAdm] Íàñòðîéêè Tracer ñîõðàíåíû!", -1)
    end
...
local function DrawTab1_Cyber()
    imgui.TextColored(cs.accent, u8">>> ÎÑÍÎÂÍÛÅ ÊÎÌÀÍÄÛ <<<")
    imgui.Spacing()
    imgui.TextWrapped(FHA_genderText(
        u8"Îñíîâíûå êîìàíäû:\n/plmenu  îòêðûòü/çàêðûòü ìåíþ\n/pl [id] [êîä/òåêñò]\n/lc - Îòêðûòü/çàêðûòü âêëàäêó Âðåìåííîå Ëèäåðñòâî\n/invis - Äëÿ èñïîëüçîâàíèÿ íåâèäèìîñòè",
        u8"Îñíîâíûå êîìàíäû:\n/plmenu  îòêðûòü/çàêðûòü ìåíþ\n/pl [id] [êîä/òåêñò]\n/lc - Îòêðûòü/çàêðûòü âêëàäêó Âðåìåííîå Ëèäåðñòâî\n/invis - Äëÿ èñïîëüçîâàíèÿ íåâèäèìîñòè"
    ))
    imgui.TextWrapped(FHA_genderText(
        u8"Áûñòðûå êîäû:\no  Îæèäàéòå\ny  Óòî÷íèòå\ngo  Óæå èäó\nhel  Ïîìîã\nsg  Ñâîáîäíàÿ ãðóïïà\nnon  Íåò â ñåòè\nper  Ïåðåäàì\notk  Îòêàç\nrp  ÐÏ ïóò¸ì\ns  Ñëåæó",
...
local function DrawTab2_Cyber()
    ...
    imgui.TextColored(cs.accent, u8">>> ÍÀÑÒÐÎÉÊÈ <<<")
    imgui.Spacing()
    ...
    if imgui.Combo(u8"Âûáåðèòå öâåò", st.menuColor, colorChoices, #colorChoices) then
        UpdateCyberColors()
    ...
    imgui.Spacing()
    imgui.Text(FHA_genderText(u8"Ïîë àäìèíèñòðàòîðà", u8"Ïîë àäìèíèñòðàòîðøè"))
    imgui.RadioButton(u8"Ìóæñêîé", st.gender, 0)
    imgui.SameLine()
    imgui.RadioButton(u8"Æåíñêèé", st.gender, 1)

    imgui.Spacing()
    imgui.Text(u8"Óðîâåíü àäìèí ïðàâ")
    imgui.SliderInt(u8"Âûáåðèòå óðîâåíü", st.adminLevel, 1, 14)

    imgui.Spacing()
    imgui.Separator()
    imgui.Spacing()
    
    imgui.TextColored(imgui.ImVec4(1, 0.8, 0.2, 1), u8"[#] Àâòîìàòè÷åñêèé âõîä â àäìèíêó")
    
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"Âêëþ÷èòü àâòî-ëîãèí", al.enabled) then
            FHA_saveCfg()
        end
    else
        imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1), u8"Âêëþ÷èòü àâòî-ëîãèí (òðåáóåòñÿ óðîâåíü 6+)")
    ...
    if al.enabled.v or st.adminLevel.v >= 6 then
        ...
        if imgui.IsItemHovered() then
            if al.showPassword.v then
                imgui.SetTooltip(u8("Ñêðûòü ïàðîëü"))
            else
                imgui.SetTooltip(u8("Ïîêàçàòü ïàðîëü"))
            end
        end
    end
...
local function DrawTab3_Cyber()
    imgui.TextColored(cs.accent, u8">>> ÎÒÂÅÒÛ ÍÀ ÐÅÏÎÐÒÛ <<<")
    imgui.Spacing()

    imgui.Text(u8"Àêòèâíûå ðåïîðòû: " .. #FHA.reports)
    imgui.SameLine()
    ...
    if #FHA.reports == 0 then
        imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1), u8"Íåò àêòèâíûõ ðåïîðòîâ")
    else
    ...
    if FHA.selectedReport then
        ...
        imgui.TextColored(cs.accent, u8"Âûáðàííûé ðåïîðò:")
        imgui.TextWrapped(string.format("%s [%d]: %s", r.nick, r.id, r.text))
        imgui.Spacing()
    else
        imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1), u8"Âûáåðèòå ðåïîðò èç ñïèñêà âûøå.")
        imgui.Spacing()
    end
    ...
    if FHA.selectedReport then
        ...
        if imgui.Button(u8"Çàêðûòü ðåïîðò", imgui.ImVec2(-1, 35)) then
            FHA_removeReport(FHA.selectedReport)
        end

        imgui.Spacing()

        if imgui.Button(u8"Îòïðàâèòü îòâåò", imgui.ImVec2(-1, 30)) then
            if #FHA.answerText.v > 0 then
                ...
                sampAddChatMessage("{FF4444}[FastHelperAdm] Ââåäèòå òåêñò îòâåòà", -1)
            end
        end
        ...
        if imgui.Button(u8"Ïðèÿòíîé Èãðû", imgui.ImVec2(buttonWidth, 28)) then
            sampSendChat(string.format("/pm %d Ïðèÿòíîé èãðû îò %s <3", r.id, FHA_adminWord()))
            FHA_removeReport(FHA.selectedReport)
        end
        
        if imgui.Button(u8"Ñïàâí", imgui.ImVec2(buttonWidth, 28)) then
            FHA_sendWithDelay(
                "/sp " .. r.id,
                string.format("/pm %d Âû óñïåøíî çàñïàâíåíû | Ïðèÿòíîé èãðû îò %s <3", r.id, FHA_adminWord()),
                950
            )
            FHA_removeReport(FHA.selectedReport)
        end
        
        if imgui.Button(u8"Îæèäàéòå", imgui.ImVec2(buttonWidth, 28)) then
            sampSendChat(string.format("/pm %d Îæèäàéòå | Ïðèÿòíîé èãðû îò %s <3", r.id, FHA_adminWord()))
            FHA_removeReport(FHA.selectedReport)
        end
        
        if imgui.Button(u8"Ïåðåäàòü", imgui.ImVec2(buttonWidth, 28)) then
            FHA_sendWithDelay(
                string.format("/a Ðåïîðò îò %s[%d] <<%s>>", r.nick, r.id, encoding.UTF8:decode(r.text)),
                string.format("/pm %d Ðåïîðò áûë ïåðåäàí | Ïðèÿòíîé èãðû îò %s <3", r.id, FHA_adminWord()),
                950
            )
            FHA_removeReport(FHA.selectedReport)
        end
        
        if imgui.Button(u8"ÑÃ", imgui.ImVec2(buttonWidth, 28)) then
            sampSendChat(string.format("/pm %d Îñòàâüòå æàëîáó â Ñâîáîäíîé Ãðóïïå ÂÊ @inferno_sv", r.id))
            FHA_removeReport(FHA.selectedReport)
        end
        imgui.EndChild()
    else
        imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1), u8"Âûáåðèòå ðåïîðò äëÿ îòîáðàæåíèÿ äåéñòâèé.")
    end
end
...
local function DrawTab4_Cyber()
    ...
    imgui.TextColored(cs.accent, u8">>> ÏÎËÅÇÍÛÅ ÔÓÍÊÖÈÈ <<<")
    imgui.Spacing()
    
    if imgui.Checkbox(u8"Àâòî Ïîæåëàíèå", st.autoWish) then st.saveSettingsFlag = true end
    imgui.SameLine()
    if imgui.Button(u8" ? ") then end
    if imgui.IsItemHovered() then
        imgui.SetTooltip(u8"Ïðè ïîÿâëåíèè PayDay áóäåò îòïðàâëåí /gg")
    end
    ...
    if st.adminLevel.v >= 6 then
                ...
                for vk = 0, 255 do
                    if wasKeyPressed(vk) then
                        ...
                        if keyName and keyName ~= "VK_0" then
                            ...
                            st.saveSettingsFlag = true
                            sampAddChatMessage(string.format("{33FF33}[Speedhack] Áèíä èçìåí¸í íà: %s", keyName), -1)
                            break
                        end
                    end
                end
        ...
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"Speedhack (Àäìèí 6+)")
    end
    
    imgui.Text(u8"Tracer")
    if imgui.Checkbox(u8"Âêëþ÷èòü Tracer", st.tracerEnabled) then st.saveSettingsFlag = true end
    imgui.SameLine()
    if imgui.Button(u8"Íàñòðîèòü Òðåéñåð") then
    ...
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"ClickWarp (òåëåïîðò íà êîëåñî)", st.clickWarpEnabled) then st.saveSettingsFlag = true end
        imgui.SameLine()
        imgui.TextDisabled(u8"(Àäìèí 6+)")
        
        if st.clickWarpEnabled.v then
            imgui.TextColored(imgui.ImVec4(0,1,0,1), u8"Óïðàâëåíèå:")
            imgui.Text(u8" Êîëåñî ìûøè - âêëþ÷èòü/âûêëþ÷èòü ðåæèì")
            imgui.Text(u8" ËÊÌ - òåëåïîðò ê òî÷êå")
            imgui.Text(u8" ÏÊÌ + ËÊÌ - òåëåïîðò â ìàøèíó")
        end
    else
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"ClickWarp [óðîâåíü 6+]")
    end
    
    imgui.Text(u8"Íåâèäèìîñòü")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"Íåâèäèìîñòü (/invis)", st.invisEnabled) then st.saveSettingsFlag = true end
        if st.invisEnabled.v then
            imgui.TextColored(imgui.ImVec4(0,1,0,1), u8("Ñòàòóñ: ") .. (st.invisActive and u8"Âêëþ÷åíî" or u8"Âûêëþ÷åíî"))
            if imgui.Button(u8"Ïåðåêëþ÷èòü íåâèäèìîñòü") then
                FHA_cmd_invis()
            end
        end
    else
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"Íåâèäèìîñòü [óðîâåíü 6+]")
    end
    
    imgui.Text(u8"AirBrake")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"Âêëþ÷èòü AirBrake (RSHIFT)", st.airbrakeEnabled) then st.saveSettingsFlag = true end
    else
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"AirBrake [óðîâåíü 6+]")
    end
    ...
    if st.adminLevel.v >= 6 then
        ...
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"GM Car [óðîâåíü 6+]")
    end

    -- ===== ZZVEH ÍÀÑÒÐÎÉÊÀ =====
    imgui.Text(u8"Îáõîä Çåë¸íîé Çîíû (ZZVeh)")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"Âêëþ÷èòü ZZVeh (ñîçäàíèå ìàøèí â ÇÇ)", st.zzvehEnabled) then
            st.saveSettingsFlag = true
            if st.zzvehEnabled.v then
                sampAddChatMessage("{33FF33}[FastHelperAdm] ZZVeh âêëþ÷åí! Ïðîñòî ïèøèòå /veh [id] [c1] [c2]", -1)
            else
                sampAddChatMessage("{FF4444}[FastHelperAdm] ZZVeh âûêëþ÷åí", -1)
            end
        end
        imgui.TextDisabled(u8"(Àäìèí 6+)")
        if st.zzvehEnabled.v then
            imgui.TextColored(imgui.ImVec4(0.5, 0.8, 1, 1), u8"Êàê ðàáîòàåò:")
            imgui.Text(u8" Ïèøèòå /veh [id] [c1] [c2] â çåë¸íîé çîíå")
            imgui.Text(u8" Âû áóäåòå òåëåïîðòèðîâàíû âíå çîíû íà 1 ñåêóíäó")
            imgui.Text(u8" Ìàøèíà ñîçäàñòñÿ íà âàøåì ìåñòå")
            imgui.Text(u8" Âû âåðí¸òåñü îáðàòíî ñ ìàøèíîé!")
        end
    else
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"ZZVeh [óðîâåíü 6+]")
    end

    imgui.Text(u8"Àâòî Ââîä êîìàíä")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"Àâòî Ââîä", st.autoEnable) then st.saveSettingsFlag = true end
        ...
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"Àâòî Ââîä [óðîâåíü 6+]")
    end
    ...
    imgui.Text("(?)")
    if imgui.IsItemHovered() then
        imgui.SetTooltip(u8"Âèäåòü ñêâîçü ñòåíû")
    end
    imgui.PopStyleColor()

    if wh_settings.enabled then
        if imgui.Checkbox(u8"Ïî íèêó", wh_by_nick_checkbox) then
            wh_settings.by_nick = wh_by_nick_checkbox.v
            st.saveSettingsFlag = true
        end
        if imgui.Checkbox(u8"Ïî ñêåëåòó", wh_by_skeleton_checkbox) then
            wh_settings.by_skeleton = wh_by_skeleton_checkbox.v
            st.saveSettingsFlag = true
        end
        if imgui.Checkbox(u8"Ïîêàçûâàòü íà ñêðèíå", wh_show_on_screenshot_checkbox) then
    ...
    if imgui.Checkbox(u8"Ïîêàçûâàòü ID â êèëë ëèñòå", imgui.ImBool(killList.enabled)) then
        killList.toggle()
    end
    imgui.SameLine()
    if imgui.Button(u8" ? ", imgui.ImVec2(25, 20)) then end
    if imgui.IsItemHovered() then
        imgui.SetTooltip(u8"Ïîêàçûâàåò ID èãðîêîâ â ñïèñêå óáèéñòâ")
    end
    ...
    if imgui.Checkbox(u8"Âêëþ÷èòü Admin Render", adminRender.enabled) then
        if adminRender.enabled.v then
            adminRender.init()
            adminRender.update()
    ...
    if adminRender.enabled.v then
        ...
        if imgui.Checkbox(u8"Ïîêàçûâàòü óðîâíè", adminRender.showLvl) then
            adminRender.saveFilter()
        end
        if imgui.Checkbox(u8"Ïîêàçûâàòü ñòàòóñû (AFK/re)", adminRender.showAction) then
            adminRender.saveFilter()
        end
        if imgui.Checkbox(u8"Ïîêàçûâàòü âðåìÿ àêòèâíîñòè", adminRender.showActive) then
            adminRender.saveFilter()
        end
        
        imgui.Spacing()
        imgui.Text(u8"Ôèëüòð ïî óðîâíÿì:")
        ...
        if imgui.Button(u8"Ïåðåìåñòèòü ñïèñîê", imgui.ImVec2(-1, 30)) then
            adminRender.startMove()
        end
        
        if adminRender.isMoving then
            adminRender.updateMove()
            imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"Íàæìèòå ENTER äëÿ ñîõðàíåíèÿ ïîçèöèè")
            imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8"Íàæìèòå ESC äëÿ îòìåíû")
        ...
        imgui.Spacing()
        imgui.Text(string.format("Ïîçèöèÿ: X=%d Y=%d", adminRender.posX, adminRender.posY))
        
        if imgui.Button(u8"Îáíîâèòü ñïèñîê ñåé÷àñ", imgui.ImVec2(-1, 25)) then
            adminRender.update()
        end
...
-- ===== ÂÊËÀÄÊÀ 5: ÀÂÒÎ ÌÅÐÎÏÐÈßÒÈÅ, ÐÀÇÄÀ×À, ÎÒÁÎÐ =====
local function DrawTab5_Cyber()
    local st = FHA.state
    
    imgui.TextColored(cs.accent, u8">>> ÀÂÒÎ ÌÅÐÎÏÐÈßÒÈÅ <<<")
    imgui.Spacing()
    
    if st.adminLevel.v < 9 then
        imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8"Äîñòóïíî ñ 9 óðîâíÿ àäìèíèñòðàòîðà")
        imgui.Separator()
        imgui.Spacing()
        imgui.TextColored(cs.accent, u8">>> ÀÂÒÎ ÐÀÇÄÀ×À <<<")
        imgui.Spacing()
        imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8"Äîñòóïíî ñ 9 óðîâíÿ àäìèíèñòðàòîðà")
        imgui.Separator()
        imgui.Spacing()
        imgui.TextColored(cs.accent, u8">>> ÀÂÒÎ ÎÒÁÎÐ <<<")
        imgui.Spacing()
        imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8"Äîñòóïíî ñ 9 óðîâíÿ àäìèíèñòðàòîðà")
        return
    end
    ...
    local mpChoices = {u8"Ñòàíäàðòíîå", u8"Ñâî¸"}
    imgui.Combo(u8"Òèï ìåðîïðèÿòèÿ", st.mp_selectEvent, mpChoices, #mpChoices)
    
    if st.mp_selectEvent.v == 0 then
        imgui.Combo(u8"Íàçâàíèå ÌÏ", st.combo_mp_name, st.mp_names, #st.mp_names)
    else
        imgui.InputText(u8"Ñâî¸ íàçâàíèå", st.mp_custom_name)
    end
    
    imgui.InputText(u8"Ïðèç", st.mp_prize_text)
    
    imgui.Spacing()
    if imgui.Button(u8"Çàïóñòèòü ìåðîïðèÿòèå", imgui.ImVec2(-1, 30)) then
    ...
    imgui.Text(u8"Ñëîâî äëÿ /rep")
    imgui.InputText("##word", st.text_word)
    
    imgui.Text(u8"×àò äëÿ îáúÿâëåíèÿ")
    ...
    if not isStyle then
        imgui.Text(u8"Êîëè÷åñòâî (5k, 1kk, 5000)")
        imgui.InputText("##amount", st.text_real)
    else
        imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"Ñòèëü áîÿ: êîë-âî íå òðåáóåòñÿ")
    end
    
    imgui.Spacing()
    if imgui.Button(u8"Çàïóñòèòü ðàçäà÷ó", imgui.ImVec2(-1, 30)) then
        st.startRazdachaFlag = true
    end
    
    imgui.Spacing()
    imgui.Separator()
    imgui.Text(u8"Ëîã ðàçäà÷:")
    
    imgui.BeginChild("##razd_log", imgui.ImVec2(0, 150), true)
    if #st.guiLog == 0 then
        imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1), u8"Ïóñòî")
    else
    ...
    imgui.Text(u8"Âûáåðèòå ëèäåðêó:")
    
    local otborChoices = {u8"Ñâî¸ íàçâàíèå", u8"Èç ñïèñêà"}
    imgui.Combo(u8"Òèï", st.otbor_selectLeader, otborChoices, #otborChoices)
    
    if st.otbor_selectLeader.v == 0 then
        imgui.InputText(u8"Íàçâàíèå ëèäåðêè", st.otbor_leader_name)
    else
        local fractionNames = {}
        for _, fr in ipairs(st.fractions) do
            table.insert(fractionNames, u8:decode(fr.name))
        end
        imgui.Combo(u8"Ôðàêöèÿ", st.otbor_leader_combo, fractionNames, #fractionNames)
    end
    ...
    imgui.Spacing()
    if imgui.Button(u8"Çàïóñòèòü îòáîð", imgui.ImVec2(-1, 30)) then
        st.startAutoOtborFlag = true
    end
end

-- ===== ÂÊËÀÄÊÀ 6: ÂÐÅÌÅÍÍÎÅ ËÈÄÅÐÑÒÂÎ =====
local function DrawTab6_Cyber()
    local st = FHA.state
    
    imgui.TextColored(cs.accent, u8">>> ÂÐÅÌÅÍÍÎÅ ËÈÄÅÐÑÒÂÎ <<<")
    imgui.Spacing()
    ...
    imgui.TextWrapped(u8"Âûáåðèòå ôðàêöèþ äëÿ ïîëó÷åíèÿ âðåìåííîãî ëèäåðñòâà. Ïðè íàæàòèè áóäåò îòïðàâëåíà êîìàíäà /templeader [íîìåð ôðàêöèè]")
    imgui.Spacing()
    imgui.Separator()
    imgui.Spacing()
...
local function DrawTab7_Cyber()
    imgui.TextColored(cs.accent, u8">>> ÏÐÀÂÈËÀ ÑÅÐÂÅÐÎÂ <<<")
    imgui.Spacing()
    ...
    imgui.InputText(u8"Ïîèñê ïî ïðàâèëàì", FHA.rulesSearch)
    ...
    if searchDecoded ~= "" then
        imgui.TextColored(imgui.ImVec4(0, 1, 0, 1), u8("Ïîèñê: ") .. u8('"' .. searchDecoded .. '"'))
        ...
        if FHA.rulesMode == 1 then
            ...
            if not found then
                imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8("Ïî çàïðîñó '" .. searchDecoded .. "' íè÷åãî íå íàéäåíî"))
            end
...
-- ===== ÂÊËÀÄÊÀ 8: ÎÁÍÎÂËÅÍÈß =====
local function DrawTab8_Cyber()
    imgui.TextColored(cs.accent, u8">>> ÎÁÍÎÂËÅÍÈß <<<")
    imgui.Spacing()
    imgui.PushTextWrapPos(imgui.GetWindowWidth() - 20)
    imgui.TextWrapped(u8(
        "FastHelperAdm Èñòîðèÿ îáíîâëåíèé:\n\n" ..
        "v1.0  Ðåëèç\n" ..
        "v1.2  Ôèêñ áàãîâ\n" ..
        "v1.4  Óëó÷øåíèÿ\n" ..
        "v1.5  Àâòî Ðàçäà÷à\n" ..
        ...
        "v1.75  Äîáàâëåí Àâòî Îòáîð è äîáàâëåí âèçóàë äëÿ ìåíþ\n" ..
        "v1.80  Ôèêñ Áàãîâ 3\n" ..
        "v1.9  Äîáàâëåí Òðåéñåð Ïóëü, ClickWarp (Òåëåïîðò íà êîë¸ñèêî ìûøè) + Âûáîð ïîëà ì/æ â íàñòðîéêàõ ìåíþ + Ôèêñ Áàãîâ + Àâòî Ïîæåëàíèå GG\n" ..
        "v2.0  Âîçâðàùåíà âêëàäêà Îòâåòû íà Ðåïîðòû + Óëó÷øåííàÿ ñèñòåìà îòâåòîâ + Äîáàâëåíà âêëàäêà Ïðàâèëà Ñåðâåðîâ (Envy/Pride/Anger) ñ ïîèñêîì + Äîáàâëåíà ôóíêöèÿ Íåâèäèìîñòè + Äîáàâëåíà ôóíêöèÿ AirBrake + Ôèêñ Àâòîðàçäà÷è\n" ..
        "v2.1  Äîáàâëåí Àâòî Alogin (6+), GM Car (GM + Âèçóàë), Íîâîå UI ìåíþ, Ïîêðàñêà Ãåòòî, Ôèêñ Àâòîðàçäà÷è#2, Speedhack, WallHack, Ôèêñ ñêåëåòà ïðè îòêðûòîì ìåíþ, Kill List ID, Admin Render, ZZVeh (îáõîä çåë¸íîé çîíû äëÿ /veh)\n"
    ));
...
local function DrawTab9_Cyber()
    imgui.TextColored(cs.accent, u8">>> ÎÁ ÀÂÒÎÐÅ <<<")
    imgui.Spacing()
    imgui.TextWrapped(
...
-- ===== ÂÊËÀÄÊÀ 10: ÏÎÊÐÀÑÊÀ ÃÅÒÒÎ =====
local function DrawTab10_Cyber()
    local st = FHA.state
    
    imgui.TextColored(cs.accent, u8">>> ÏÎÊÐÀÑÊÀ ÃÅÒÒÎ <<<")
    imgui.Spacing()
    ...
    if st.gz_autoReform.active then
        if st.gz_autoReform.paused then
            if imgui.Button(u8("ÏÐÎÄÎËÆÈÒÜ"), imgui.ImVec2(120, 30)) then GZ_pauseAutoReform() end
        else
            if imgui.Button(u8("ÏÀÓÇÀ"), imgui.ImVec2(120, 30)) then GZ_pauseAutoReform() end
        end
        imgui.SameLine()
        if imgui.Button(u8("ÑÒÎÏ"), imgui.ImVec2(120, 30)) then GZ_stopAutoReform() end
        imgui.Spacing()
        imgui.TextColored(imgui.ImVec4(0, 1, 0, 1), string.format("Progress: %d/%d", st.gz_autoReform.currentIndex - 1, #allReformZones))
    else
        if imgui.Button(u8("ÑÒÀÐÒ"), imgui.ImVec2(120, 30)) then
        ...
        for colIndex, zoneId in ipairs(row) do
            ...
            if zone then
                ...
                if imgui.IsItemHovered() then
                    ...
                    imgui.Text(u8"ËÊÌ - Òåëåïîðò")
                    imgui.Text(u8"ÏÊÌ - Ïîêðàñèòü")
                    if isUnderCap then 
...
function FHA_doAutoMP()
    ...
    if prizeText == "" then
        sampAddChatMessage("{FF4444}[MP] Óêàæèòå ïðèç ìåðîïðèÿòèÿ", -1)
        return
    end
    ...
    wait(1000)
    sampSendChat('/aad MP | Ïðèç: '..prizeText)
    wait(1000)
...
function FHA_doAutoOtbor()
    ...
    if st.otbor_selectLeader.v == 1 then
        ...
        if selectedFractionIndex >= 1 and selectedFractionIndex <= #st.fractions then
            ...
            sampAddChatMessage("{FF4444}[Îòáîð] Îøèáêà: Íåâåðíûé èíäåêñ ôðàêöèè â ñïèñêå.", -1)
            return
        end
    else
    ...
    sampSendChat('/'..chatCmd..' ÎÒÁÎÐ | Ñåé÷àñ ïðîéä¸ò îòáîð íà ëèäåðà "'..leaderNameCP1251..'"')
    wait(1000)
...
function FHA_doRazdacha()
    ...
    if st.razdInProgress then
        sampAddChatMessage("{FF4444}[FastHelperAdm] Ðàçäà÷à óæå çàïóùåíà! Äîæäèòåñü îêîí÷àíèÿ.", -1)
        return
    end
    ...
    if st.text_word.v == "" or st.text_word.v == " " then
        sampAddChatMessage("{FF4444}[FastHelperAdm] Óêàæèòå ñëîâî äëÿ /rep", -1)
        return
    end
    
    if not isStyle and (amount == nil or amount <= 0) then
        sampAddChatMessage("{FF4444}[FastHelperAdm] Óêàæèòå êîððåêòíîå êîëè÷åñòâî (ïðèìåð: 5k, 1kk, 5000)", -1)
        return
    end
    
    local txt
    if isStyle then
        txt = "ÐÀÇÄÀ×À | Êòî ïåðâûé íàïèøåò /rep "..u8:decode(st.text_word.v).." ïîëó÷èò ñòèëü \""..pName.."\""
    else
        txt = "ÐÀÇÄÀ×À | Êòî ïåðâûé íàïèøåò /rep "..u8:decode(st.text_word.v).." ïîëó÷èò "..formatMoneySmart(amount).." "..pName
    end
    ...
    sampAddChatMessage("{33CCFF}[FastHelperAdm] Ðàçäà÷à çàïóùåíà! Îæèäàíèå îòâåòà 30 ñåêóíä...", -1)
end
...
function FHA_toggleAirBrake()
    ...
    if not st.airbrakeEnabled.v then
        sampAddChatMessage("{FF4444}[FastHelperAdm] Ôóíêöèÿ AirBrake íå àêòèâèðîâàíà â ìåíþ ñêðèïòà.", -1)
        return
    end
    if st.adminLevel.v < 6 then
        sampAddChatMessage("{FF4444}[FastHelperAdm] Äëÿ èñïîëüçîâàíèÿ AirBrake òðåáóåòñÿ óðîâåíü àäìèíèñòðàòîðà 6+.", -1)
        return
    end
...
function FHA_cmd_pl(param)
    ...
    if now - st.lastSendTime < st.cooldown then
        sampAddChatMessage("{FF0000}[FastReply] Ïîäîæäèòå íåìíîãî", -1)
        return
    end
    st.lastSendTime = now
    if not param or param == "" then
        sampAddChatMessage("{FF0000}Èñïîëüçîâàíèå: /pl [id] [êîä/òåêñò]", -1)
        return
    end
    ...
    if not id or not sampIsPlayerConnected(id) then
        sampAddChatMessage("{FF0000}Îøèáêà: èãðîê íå íàéäåí", -1)
        return
    end
...
function FHA_cmd_invis()
    ...
    if not st.invisEnabled.v then
        sampAddChatMessage("{FF4444}[FastHelperAdm] Ôóíêöèÿ íåâèäèìîñòè íå àêòèâèðîâàíà â ìåíþ ñêðèïòà.", -1)
        return
    end
    if st.adminLevel.v < 6 then
        sampAddChatMessage("{FF4444}[FastHelperAdm] Äëÿ èñïîëüçîâàíèÿ íåâèäèìîñòè òðåáóåòñÿ óðîâåíü àäìèíèñòðàòîðà 6+.", -1)
        return
    end

    st.invisActive = not st.invisActive
    sampAddChatMessage("[{FF0000}FastHelperAdm{FFFFFF}] {ffff00}Íåâèäèìîñòü " .. (st.invisActive and "{00ff00}Âêëþ÷åíî" or "{FF0000}Âûêëþ÷åíî"), -1)
end
...
function FHA_updateSystem()
    ...
    if st.active_razd and not st.active_razd2 and st.razdTimeout > 0 and os.clock() > st.razdTimeout then
        sampAddChatMessage("{FF5555}[FastHelperAdm] Ðàçäà÷à îòìåíåíà - íèêòî íå ââåë /rep " .. u8:decode(st.text_word.v), -1)
        FHA_resetRazdacha()
    end
        ...
        if not sampIsPlayerConnected(st.razd_player_id) then
            ...
            lua_thread.create(function()
                ...
                local pm_message = isStyle
                    and ('Ïîçäðàâëÿåì! Âû ïîáåäèëè â ðàçäà÷å! Âû âûèãðàëè ñòèëü áîÿ "'..prize..'"')
                    or  ('Ïîçäðàâëÿåì! Âû ïîáåäèëè â ðàçäà÷å! Âûèãðàëè '..formatMoneySmart(amount)..' '..prize)
                ...
                local announce_message = isStyle
                    and ('ÐÀÇÄÀ×À | WIN '..st.razd_player_id..'id âûèãðàë ñòèëü "'..prize..'"')
                    or  ('ÐÀÇÄÀ×À | WIN '..st.razd_player_id..'id')
...
function imgui.OnDrawFrame()
    ...
    if not success then
        ...
        ShowCursor(false)
        imgui.Process = false
        print("[FastHelperAdm][Imgui] Îøèáêà ðèñîâàíèÿ:", err)
    end
end
...
function script.unload()
    ...
    sampAddChatMessage("{33CCFF}[FastHelperAdm] Ñêðèïò âûãðóæåí êîððåêòíî", -1)
end
...
function main()
    ...
    sampAddChatMessage("{CCCCCC}[INFORMATION] {CC88FF}Ñêðèïò {AA66FF}FastHelperAdm {999999}version 2.1 {CC88FF}óñïåøíî çàãðóæåí", -1)
    sampAddChatMessage("{CCCCCC}[INFORMATION] {CC88FF}Äëÿ èñïîëüçîâàíèÿ ïðîïèøèòå - {999999}/plmenu", -1)
    ...
    while not FHA.isUnloading do
        ...
        if not success then
            print("[FastHelperAdm][Input] Îøèáêà â Input:update():", err)
            Input:clear()
        end
