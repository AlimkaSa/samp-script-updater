...
require "lib.render"
local dlstatus = require('moonloader').download_status -- ???????? ???????? dowload_status
...
function adminRender.onServerMessage(color, text)
    ...
    -- ?????????? ?????????
    if cleanText:find("????????????? ? ????:") or cleanText:find("?????????????? ??????") then
        return true
    end
    
    -- ??????: "Nickname[ID] (LVL lvl) [AFK]"
...
function adminRender.draw()
    ...
    if #adminRender.admins == 0 then
        ...
        renderFontDrawText(adminRender.font, "??????????????: ????????...", posX, posY, -1)
        return
    end
    ...
    local header = string.format("?????????????? {00ff00}online{ffffff} [ %s | {ff0000}AFK: %s{ffffff} | {32CD32}/re: %s{ffffff} ]:", 
...
FHA.state = {
    ...
    arr_priz = {
        ...
        u8'????? ????????',u8'EXP',u8'?????? ? ?????',
        u8'?????? ?? ??????',u8'???????? ??????',u8'???????',
        u8'????',u8'Kung-Fu',u8'KickBox',u8'????????????????',u8'?????????'
    },
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    ...
    if st.mpAutoStep == 1 and title:find(u8:decode("???? ???????????")) then
        lua_thread.create(function()
            if FHA.isUnloading then return end
            wait(200)
            if FHA.isUnloading then return end
    ...
    if st.otborRunning and title:find(u8:decode("???? ???????????")) then
        lua_thread.create(function()
            if FHA.isUnloading then return end
            wait(200)
            if FHA.isUnloading then return end
...
function sampev.onServerMessage(color, text)
    ...
    -- ????????? ????? ?? ???? (??? ? GrandTools)
    if rawText:find("?? ???????? ??????? ????") or rawText:find("?? ???????? ????") then
    ...
    if state.autoWish.v then
        ...
        elseif not cleanText:find("?????????? ???") then
    ...
    if state.active_razd and not state.active_razd2 and state.text_word.v ~= "" then
        local _, pid2, msg2 = text:match('?????? ?? (.*)%[(%d+)%]: %{FFFFFF%}(.*)')
        if msg2 then
            ...
            if repWord == u8:decode(state.text_word.v) then
                if sampIsPlayerConnected(tonumber(pid2)) then
                    ...
                    sampAddChatMessage("{33FF33}[FastHelperAdm] ?????????? ??????! ?????? ?????...", -1)
                else
                    sampAddChatMessage("{FF5555}[FastHelperAdm] ?????????? ????? ?? ????, ??????? ????????", -1)
                    FHA_resetRazdacha()
                end
            end
        end
...
local function DrawTracerSettings()
    ...
    imgui.TextColored(cs.accent, u8"=== ????????? ???????? ===")
    imgui.Spacing()
    
    imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"??? ????:")
    if imgui.Checkbox(u8"?????????? ???? ????", st.tracerDrawMyBullets) then st.saveSettingsFlag = true end
    if imgui.Checkbox(u8"??????? ? ????? (????)", st.tracerCbEndMy) then st.saveSettingsFlag = true end
    
    imgui.SliderInt(u8"????? ??????????? (????)", st.tracerTimeRenderMyBullets, 1, 60)
    imgui.SliderInt(u8"??????? ????? (????)", st.tracerSizeOffMyLine, 1, 10)
    imgui.SliderInt(u8"?????? ???????? (????)", st.tracerSizeOffMyPolygonEnd, 1, 20)
    imgui.SliderInt(u8"???????? ???????? (????)", st.tracerRotationMyPolygonEnd, 1, 360)
    imgui.SliderInt(u8"???? ???????? (????)", st.tracerDegreeMyPolygonEnd, 3, 360)
    imgui.SliderInt(u8"????? ????? (????)", st.tracerMaxLineMyLimit, 1, 100)
    
    imgui.ColorEdit4(u8"??????. ?????? (????)", st.tracerStaticObjectMy)
    imgui.ColorEdit4(u8"???????. ?????? (????)", st.tracerDinamicObjectMy)
    imgui.ColorEdit4(u8"????? (????)", st.tracerPedPMy)
    imgui.ColorEdit4(u8"?????? (????)", st.tracerCarPMy)
    
    imgui.Separator()
    imgui.Spacing()
    
    imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"????? ????:")
    if imgui.Checkbox(u8"?????????? ????? ????", st.tracerDrawBullets) then st.saveSettingsFlag = true end
    if imgui.Checkbox(u8"??????? ? ????? (?????)", st.tracerCbEnd) then st.saveSettingsFlag = true end
    if imgui.Checkbox(u8"?????????? ?? ??????", st.tracerShowPlayerInfo) then st.saveSettingsFlag = true end
    
    if st.tracerShowPlayerInfo.v then
        if imgui.Checkbox(u8"?????? ID", st.tracerOnlyId) then st.saveSettingsFlag = true end
        imgui.SameLine()
        if imgui.Checkbox(u8"?????? ???", st.tracerOnlyNick) then st.saveSettingsFlag = true end
    end
    
    imgui.SliderInt(u8"????? ??????????? (?????)", st.tracerTimeRenderBullets, 1, 60)
    imgui.SliderInt(u8"??????? ????? (?????)", st.tracerSizeOffLine, 1, 10)
    imgui.SliderInt(u8"?????? ???????? (?????)", st.tracerSizeOffPolygonEnd, 1, 20)
    imgui.SliderInt(u8"???????? ???????? (?????)", st.tracerRotationPolygonEnd, 1, 360)
    imgui.SliderInt(u8"???? ???????? (?????)", st.tracerDegreePolygonEnd, 3, 360)
    imgui.SliderInt(u8"????? ????? (?????)", st.tracerMaxLineLimit, 1, 100)
    
    imgui.ColorEdit4(u8"??????. ?????? (?????)", st.tracerStaticObject)
    imgui.ColorEdit4(u8"???????. ?????? (?????)", st.tracerDinamicObject)
    imgui.ColorEdit4(u8"????? (?????)", st.tracerPedP)
    imgui.ColorEdit4(u8"?????? (?????)", st.tracerCarP)
    imgui.ColorEdit4(u8"???? ???? ??????", st.tracerColorPlayerI)
    ...
    if imgui.Button(u8"????????? ?????????", imgui.ImVec2(-1, 30)) then
        FHA_saveCfg()
        sampAddChatMessage("{33FF33}[FastHelperAdm] ????????? Tracer ?????????!", -1)
    end
...
local function DrawTab2_Cyber()
    ...
    if imgui.Combo(u8"???????? ????", st.menuColor, colorChoices, #colorChoices) then
        UpdateCyberColors()
        st.saveSettingsFlag = true
    end

    if st.menuColor.v == 7 then
        imgui.TextColored(imgui.ImVec4(1,0,1,1), u8"? ???????? ????? ???????")
    end

    imgui.Spacing()
    imgui.Text(FHA_genderText(u8"??? ??????????????", u8"??? ???????????????"))
    imgui.RadioButton(u8"???????", st.gender, 0)
    imgui.SameLine()
    imgui.RadioButton(u8"???????", st.gender, 1)

    imgui.Spacing()
    imgui.Text(u8"??????? ????? ????")
    imgui.SliderInt(u8"???????? ???????", st.adminLevel, 1, 14)

    imgui.Spacing()
    imgui.Separator()
    imgui.Spacing()
    
    imgui.TextColored(imgui.ImVec4(1, 0.8, 0.2, 1), u8"[#] ?????????????? ???? ? ???????")
    
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"???????? ????-?????", al.enabled) then
            FHA_saveCfg()
        end
    else
        imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1), u8"???????? ????-????? (????????? ??????? 6+)")
    ...
    if st.adminLevel.v >= 9 then
        ...
        imgui.TextColored(imgui.ImVec4(1,0.5,0,1), u8"?????? ?????????. ?????? 1-8 ?? ????? ????????????:")
        imgui.Text(u8"• ????-???????????")
        imgui.Text(u8"• ????-???????") 
        imgui.Text(u8"• ????-?????")
        imgui.Text(u8"• ????????? ?????????")
    ...
    if FHA.selectedReport then
        ...
        if imgui.Button(u8"????????? ?????", imgui.ImVec2(-1, 30)) then
            if #FHA.answerText.v > 0 then
                ...
                sampAddChatMessage("{FF4444}[FastHelperAdm] ??????? ????? ??????", -1)
            end
        end
        ...
        if imgui.Button(u8"???????? ????", imgui.ImVec2(buttonWidth, 28)) then
            sampSendChat(string.format("/pm %d ???????? ???? ?? %s <3", r.id, FHA_adminWord()))
            FHA_removeReport(FHA.selectedReport)
        end
        ...
        if imgui.Button(u8"?????", imgui.ImVec2(buttonWidth, 28)) then
            FHA_sendWithDelay(
                "/sp " .. r.id,
                string.format("/pm %d ?? ??????? ?????????? | ???????? ???? ?? %s <3", r.id, FHA_adminWord()),
                950
            )
            FHA_removeReport(FHA.selectedReport)
        end
        ...
        if imgui.Button(u8"????????", imgui.ImVec2(buttonWidth, 28)) then
            FHA_sendWithDelay(
                string.format("/a ?????? ?? %s[%d] <<%s>>", r.nick, r.id, encoding.UTF8:decode(r.text)),
                string.format("/pm %d ?????? ??? ??????? | ???????? ???? ?? %s <3", r.id, FHA_adminWord()),
                950
            )
            FHA_removeReport(FHA.selectedReport)
        ...
        if imgui.Button(u8"??", imgui.ImVec2(buttonWidth, 28)) then
            sampSendChat(string.format("/pm %d ???????? ?????? ? ????????? ?????? ?? @inferno_sv", r.id))
            FHA_removeReport(FHA.selectedReport)
        end
        imgui.EndChild()
    else
...
local function DrawTab4_Cyber()
    ...
    if imgui.Checkbox(u8"???? ?????????", st.autoWish) then st.saveSettingsFlag = true end
    imgui.SameLine()
    if imgui.Button(u8" ? ") then end
    if imgui.IsItemHovered() then
        imgui.SetTooltip(u8"??? ????????? PayDay ????? ????????? /gg")
    end
    ...
    if st.adminLevel.v >= 6 then
                ...
                for vk = 0, 255 do
                    if wasKeyPressed(vk) then
                        ...
                        if keyName and keyName ~= "VK_0" then
                            ...
                            st.saveSettingsFlag = true
                            sampAddChatMessage(string.format("{33FF33}[Speedhack] ???? ??????? ??: %s", keyName), -1)
                            break
                        end
                    end
                end
        ...
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"Speedhack (????? 6+)")
    end
    
    imgui.Text(u8"Tracer")
    if imgui.Checkbox(u8"???????? Tracer", st.tracerEnabled) then st.saveSettingsFlag = true end
    imgui.SameLine()
    if imgui.Button(u8"????????? ???????") then
    ...
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"ClickWarp (???????? ?? ??????)", st.clickWarpEnabled) then st.saveSettingsFlag = true end
        imgui.SameLine()
        imgui.TextDisabled(u8"(????? 6+)")
        ...
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"ClickWarp [??????? 6+]")
    end
    
    imgui.Text(u8"???????????")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"??????????? (/invis)", st.invisEnabled) then st.saveSettingsFlag = true end
        if st.invisEnabled.v then
            imgui.TextColored(imgui.ImVec4(0,1,0,1), u8("??????: ") .. (st.invisActive and u8"????????" or u8"?????????"))
            if imgui.Button(u8"??????????? ???????????") then
                FHA_cmd_invis()
            end
        end
    else
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"??????????? [??????? 6+]")
    end
    
    imgui.Text(u8"AirBrake")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"???????? AirBrake (RSHIFT)", st.airbrakeEnabled) then st.saveSettingsFlag = true end
    else
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"AirBrake [??????? 6+]")
    end
    ...
    if st.adminLevel.v >= 6 then
        ...
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"GM Car [??????? 6+]")
    end

    -- ===== ZZVEH ????????? =====
    imgui.Text(u8"????? ??????? ???? (ZZVeh)")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"???????? ZZVeh (???????? ????? ? ??)", st.zzvehEnabled) then
            st.saveSettingsFlag = true
            if st.zzvehEnabled.v then
                sampAddChatMessage("{33FF33}[FastHelperAdm] ZZVeh ???????! ?????? ?????? /veh [id] [c1] [c2]", -1)
            else
                sampAddChatMessage("{FF4444}[FastHelperAdm] ZZVeh ????????", -1)
            end
        end
        imgui.TextDisabled(u8"(????? 6+)")
        if st.zzvehEnabled.v then
            imgui.TextColored(imgui.ImVec4(0.5, 0.8, 1, 1), u8"??? ????????:")
            imgui.Text(u8"• ?????? /veh [id] [c1] [c2] ? ??????? ????")
            imgui.Text(u8"• ?? ?????? ??????????????? ??? ???? ?? 1 ???????")
            imgui.Text(u8"• ?????? ????????? ?? ????? ?????")
            imgui.Text(u8"• ?? ????????? ??????? ? ???????!")
        end
    else
        imgui.TextColored(imgui.ImVec4(0.5,0.5,0.5,1), u8"ZZVeh [??????? 6+]")
    end

    imgui.Text(u8"???? ???? ??????")
    if st.adminLevel.v >= 6 then
        if imgui.Checkbox(u8"???? ????", st.autoEnable) then st.saveSettingsFlag = true end
    ...
    if wh_settings.enabled then
        if imgui.Checkbox(u8"?? ????", wh_by_nick_checkbox) then
            wh_settings.by_nick = wh_by_nick_checkbox.v
            st.saveSettingsFlag = true
        end
        if imgui.Checkbox(u8"?? ???????", wh_by_skeleton_checkbox) then
            wh_settings.by_skeleton = wh_by_skeleton_checkbox.v
            st.saveSettingsFlag = true
        end
        if imgui.Checkbox(u8"?????????? ?? ??????", wh_show_on_screenshot_checkbox) then
    ...
    if imgui.Checkbox(u8"?????????? ID ? ???? ?????", imgui.ImBool(killList.enabled)) then
        killList.toggle()
    end
    imgui.SameLine()
    if imgui.Button(u8" ? ", imgui.ImVec2(25, 20)) then end
    if imgui.IsItemHovered() then
        imgui.SetTooltip(u8"?????????? ID ??????? ? ?????? ???????")
    end
    ...
    if imgui.Checkbox(u8"???????? Admin Render", adminRender.enabled) then
        if adminRender.enabled.v then
            adminRender.init()
            adminRender.update()
    ...
    if adminRender.enabled.v then
        ...
        if imgui.Checkbox(u8"?????????? ??????", adminRender.showLvl) then
            adminRender.saveFilter()
        end
        if imgui.Checkbox(u8"?????????? ??????? (AFK/re)", adminRender.showAction) then
            adminRender.saveFilter()
        end
        if imgui.Checkbox(u8"?????????? ????? ??????????", adminRender.showActive) then
            adminRender.saveFilter()
        end
        ...
        if adminRender.isMoving then
            ...
            imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8"??????? ENTER ??? ?????????? ???????")
            imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8"??????? ESC ??? ??????")
        ...
        imgui.Spacing()
        imgui.Text(string.format("???????: X=%d Y=%d", adminRender.posX, adminRender.posY))
...
local function DrawTab5_Cyber()
    ...
    if st.adminLevel.v < 9 then
        imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8"???????? ? 9 ?????? ??????????????")
        imgui.Separator()
        imgui.Spacing()
        imgui.TextColored(cs.accent, u8">>> ???? ??????? <<<")
        imgui.Spacing()
    ...
    local mpChoices = {u8"???????????", u8"????"}
    imgui.Combo(u8"??? ???????????", st.mp_selectEvent, mpChoices, #mpChoices)
    
    if st.mp_selectEvent.v == 0 then
        imgui.Combo(u8"???????? ??", st.combo_mp_name, st.mp_names, #st.mp_names)
    else
        imgui.InputText(u8"???? ????????", st.mp_custom_name)
    end
    
    imgui.InputText(u8"????", st.mp_prize_text)
    ...
    local otborChoices = {u8"???? ????????", u8"?? ??????"}
    imgui.Combo(u8"???", st.otbor_selectLeader, otborChoices, #otborChoices)
    
    if st.otbor_selectLeader.v == 0 then
        imgui.InputText(u8"???????? ???????", st.otbor_leader_name)
    else
        local fractionNames = {}
        for _, fr in ipairs(st.fractions) do
            table.insert(fractionNames, u8:decode(fr.name))
        end
        imgui.Combo(u8"???????", st.otbor_leader_combo, fractionNames, #fractionNames)
    end
...
local function DrawTab6_Cyber()
    ...
    imgui.TextWrapped(u8"???????? ??????? ??? ????????? ?????????? ?????????. ??? ??????? ????? ?????????? ??????? /templeader [????? ???????]")
    imgui.Spacing()
    imgui.Separator()
    imgui.Spacing()
...
local function DrawTab7_Cyber()
    ...
    imgui.InputText(u8"????? ?? ????????", FHA.rulesSearch)
    ...
    if searchDecoded ~= "" then
        imgui.TextColored(imgui.ImVec4(0, 1, 0, 1), u8("?????: ") .. u8('"' .. searchDecoded .. '"'))
        ...
        if FHA.rulesMode == 1 then
            ...
            if not found then
                imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8("?? ??????? '" .. searchDecoded .. "' ?????? ?? ???????"))
            end
...
local function DrawTab8_Cyber()
    ...
    imgui.TextWrapped(u8(
        ...
        "v1.80 — ???? ????? 3\n" ..
        "v1.9 — ???????? ??????? ????, ClickWarp (???????? ?? ???????? ????) + ????? ???? ?/? ? ?????????? ???? + ???? ????? + ???? ????????? GG\n" ..
        "v2.0 — ?????????? ??????? ?????? ?? ??????? + ?????????? ??????? ??????? + ????????? ??????? ??????? ???????? (Envy/Pride/Anger) ? ??????? + ????????? ??????? ??????????? + ????????? ??????? AirBrake + ???? ???????????\n" ..
        "v2.1 — ???????? ???? Alogin (6+), GM Car (GM + ??????), ????? UI ????, ???????? ?????, ???? ???????????#2, Speedhack, WallHack, ???? ??????? ??? ???????? ????, Kill List ID, Admin Render, ZZVeh (????? ??????? ???? ??? /veh)\n"
    ));
...
local function DrawTab10_Cyber()
    ...
    if st.gz_autoReform.active then
        if st.gz_autoReform.paused then
            if imgui.Button(u8("??????????"), imgui.ImVec2(120, 30)) then GZ_pauseAutoReform() end
        else
            if imgui.Button(u8("?????"), imgui.ImVec2(120, 30)) then GZ_pauseAutoReform() end
        end
        imgui.SameLine()
        if imgui.Button(u8("????"), imgui.ImVec2(120, 30)) then GZ_stopAutoReform() end
        imgui.Spacing()
...
function FHA_doAutoMP()
    ...
    if st.mp_selectEvent.v == 0 then
        ...
        if st.mp_custom_name.v ~= "" then
            ...
            sampAddChatMessage("{FF4444}[MP] ??????? ???????? ???????????", -1)
            return
        end
    end
    ...
    sampSendChat('/aad MP | ????????? ??????, ?????? ??????? ??????????? "'..mpName..'"')
    wait(1000)
    sampSendChat('/aad MP | ????: '..prizeText)
    wait(1000)
...
function FHA_doAutoOtbor()
    ...
    sampSendChat('/'..chatCmd..' ????? | ?????? ??????? ????? ?? ?????? "'..leaderNameCP1251..'"')
    wait(1000)

    sampSendChat('/'..chatCmd..' ????? | ????????: 2+ ????? ?? ????????, ????? ??')
    wait(1000)

    sampSendChat('/'..chatCmd..' ????? | ???????? /gomp')
    wait(1000)
...
function FHA_doRazdacha()
    ...
    if st.text_word.v == "" or st.text_word.v == " " then
        sampAddChatMessage("{FF4444}[FastHelperAdm] ??????? ????? ??? /rep", -1)
        return
    end
    
    if not isStyle and (amount == nil or amount <= 0) then
        sampAddChatMessage("{FF4444}[FastHelperAdm] ??????? ?????????? ?????????? (??????: 5k, 1kk, 5000)", -1)
        return
    end
    
    local txt
    if isStyle then
        txt = "??????? | ??? ?????? ??????? /rep "..u8:decode(st.text_word.v).." ??????? ????? \""..pName.."\""
    else
        txt = "??????? | ??? ?????? ??????? /rep "..u8:decode(st.text_word.v).." ??????? "..formatMoneySmart(amount).." "..pName
    end
    ...
    sampAddChatMessage("{33CCFF}[FastHelperAdm] ??????? ????????! ???????? ?????? 30 ??????...", -1)
end
...
function FHA_toggleAirBrake()
    ...
    if not st.airbrakeEnabled.v then
        sampAddChatMessage("{FF4444}[FastHelperAdm] ??????? AirBrake ?? ???????????? ? ???? ???????.", -1)
        return
    end
    if st.adminLevel.v < 6 then
        sampAddChatMessage("{FF4444}[FastHelperAdm] ??? ????????????? AirBrake ????????? ??????? ?????????????? 6+.", -1)
        return
    end
...
function FHA_cmd_pl(param)
    ...
    if now - st.lastSendTime < st.cooldown then
        sampAddChatMessage("{FF0000}[FastReply] ????????? ???????", -1)
        return
    end
    st.lastSendTime = now
    if not param or param == "" then
        sampAddChatMessage("{FF0000}?????????????: /pl [id] [???/?????]", -1)
        return
    end
...
function FHA_cmd_invis()
    ...
    if st.adminLevel.v < 6 then
        sampAddChatMessage("{FF4444}[FastHelperAdm] ??? ????????????? ??????????? ????????? ??????? ?????????????? 6+.", -1)
        return
    end

    st.invisActive = not st.invisActive
    sampAddChatMessage("[{FF0000}FastHelperAdm{FFFFFF}] {ffff00}??????????? " .. (st.invisActive and "{00ff00}????????" or "{FF0000}?????????"), -1)
end
...
function FHA_updateSystem()
    ...
    if st.active_razd and not st.active_razd2 and st.razdTimeout > 0 and os.clock() > st.razdTimeout then
        sampAddChatMessage("{FF5555}[FastHelperAdm] ??????? ???????? - ????? ?? ???? /rep " .. u8:decode(st.text_word.v), -1)
        FHA_resetRazdacha()
    end
        ...
        if not sampIsPlayerConnected(st.razd_player_id) then
            ...
            lua_thread.create(function()
                ...
                local pm_message = isStyle
                    and ('???????????! ?? ???????? ? ???????! ?? ???????? ????? ??? "'..prize..'"')
                    or  ('???????????! ?? ???????? ? ???????! ???????? '..formatMoneySmart(amount)..' '..prize)
                ...
                local announce_message = isStyle
                    and ('??????? | WIN '..st.razd_player_id..'id ??????? ????? "'..prize..'"')
                    or  ('??????? | WIN '..st.razd_player_id..'id')
...
function script.unload()
    ...
    sampAddChatMessage("{33CCFF}[FastHelperAdm] ?????? ???????? ?????????", -1)
end
...
function main()
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if updateIni and tonumber(updateIni.info.vers) > script_vers then
                sampAddChatMessage("{33FF33}[FastHelperAdm] ???? ??????????! ??????: " .. updateIni.info.vers_text, -1)
                update_state = true
            end
    ...
    sampAddChatMessage("{CCCCCC}[INFORMATION] {CC88FF}?????? {AA66FF}FastHelperAdm {999999}version 2.2 {CC88FF}??????? ????????", -1)
    sampAddChatMessage("{CCCCCC}[INFORMATION] {CC88FF}??? ????????????? ????????? - {999999}/plmenu", -1)
    ...
    while not FHA.isUnloading do
        ...
        if not success then
            print("[FastHelperAdm][Input] ?????? ? Input:update():", err)
            Input:clear()
        end
